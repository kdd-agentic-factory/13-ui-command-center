...
function AgentBadge({ agent }: { agent: LogAgent }) {
  const map: Record<LogAgent, { label: string; bg: string; color: string }> = {
    ...
    SYS: { label: '─¢─—──¾—¢', bg: 'var(--surface-soft)', color: 'var(--text-muted)' },
  };
  const s = map[agent];
  return (
    <span style={{
...
// ──── Generate pipeline ────
  const handleGenerate = useCallback(async () => {
    ...
    // Physics
    const vol      = P.dimX * P.dimY * P.dimZ * 1e-9; // m─š─—³
...
// ──── PHASE 1 Design Agent ────
    ...
    A('D', `Material: ${mat.label}  ───—=${mat.density} kg/m─š─—³  ───—─_yield=${mat.yieldMPa} MPa`);
    await S(400); F('ok');
...
// ──── PHASE 2 Validator Agent ────
    ...
    A('V', `Aerodynamic drag: Cd = ${dragCd} (OpenFOAM k-── — ° SST)`);
    await S(680); F('ok');
...
; Mass  : ${result.mass} kg  |  Peak ───—─: ${result.peakStress} MPa
