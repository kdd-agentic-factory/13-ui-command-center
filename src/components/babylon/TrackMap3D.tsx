...
export function TrackMap3D({ trackPos, height = 300 }: TrackMap3DProps) {
  ...
  useEffect(() => {
    ...
    const pts3D = TRACK_POINTS.map(([x, z]) => new Vector3(x * 1.5, 0, z * 1.5));
    ...
    for (let i = 0; i < pts3D.length - 1; i++) {
      ...
      for (const side of [-0.32, 0.32]) {
        ...
        edge.position = new Vector3(mid.x + Math.cos(angle + Math.PI / 2) * side, 0.01, mid.z + Math.sin(angle + Math.PI / 2) * side);
