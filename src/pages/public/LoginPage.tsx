  ...
  return (
    <main style={{ minHeight: '100vh', padding: 24, background: 'radial-gradient(circle at top, rgba(59,130,246,0.14), transparent 28%), #070b14', color: 'var(--color-text, #eef1f8)' }}>
      <section style={{ width: 'min(980px, 100%)', margin: '0 auto', display: 'grid', gap: 18 }}>
        <div style={{ border: '1px solid rgba(148,163,184,0.18)', borderRadius: 24, padding: '28px clamp(20px, 4vw, 34px)', background: 'rgba(15,23,42,0.76)', boxShadow: '0 24px 80px rgba(0,0,0,0.32)' }}>
          <p style={{ margin: 0, color: '#93c5fd', textTransform: 'uppercase', letterSpacing: '0.14em', fontSize: 11, fontWeight: 700 }}>{copy.eyebrow}</p>
          <h1 style={{ margin: '10px 0 12px', fontSize: 'clamp(32px, 5vw, 52px)', lineHeight: 1.02, maxWidth: 820 }}>{copy.title}</h1>
          <p style={{ margin: '0 0 18px', color: 'var(--color-text-muted, #98a2b3)', lineHeight: 1.7, maxWidth: 760 }}>
            {copy.body}
          </p>
          <p style={{ margin: 0, color: 'var(--color-text-muted, #98a2b3)', lineHeight: 1.7, maxWidth: 760 }}>
            {copy.note}
          </p>
        </div>
        ...
        <nav style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center', color: 'var(--color-text-muted, #98a2b3)' }}>
