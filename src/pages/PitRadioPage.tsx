  ...
  return (
    <div className="page">
      <div className="flex items-center justify-between mb-6">
        ...
        <span style={{ fontSize: 9.5, fontFamily: MONO, color: 'var(--yellow)', border: '1px solid rgba(252,211,77,0.3)', borderRadius: 4, padding: '2px 7px' }}>RADIO LOG · no live audio</span>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 360, overflowY: 'auto' }}>
            {transcript.map((m, i) => (
              <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                ...
                <span style={{ fontSize: 12, color: 'var(--text)', lineHeight: 1.4, borderLeft: m.priority === 'high' ? '2px solid var(--accent)' : 'none', paddingLeft: m.priority === 'high' ? 8 : 0 }}>{m.text}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
              {base.canned.map(c => (
                <button key={c.id} onClick={() => send(c.text)} title={c.text}
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontFamily: MONO, color: 'var(--cyan)', background: 'rgba(0,183,255,0.08)', border: '1px solid rgba(0,183,255,0.3)', borderRadius: 6, padding: '5px 9px', cursor: 'pointer' }}>
          ...
          <div className="card" style={{ padding: 16 }}>
            ...
            {base.commands.map(c => (
              <div key={c.phrase} style={{ display: 'flex', gap: 10, fontSize: 11.5, marginBottom: 7 }}>
                <span style={{ color: '#8B5CF6', fontStyle: 'italic', flexShrink: 0 }}>{c.phrase}</span>
