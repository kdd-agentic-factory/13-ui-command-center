      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="card" style={{ padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              ...
              <button onClick={runTwin} disabled={validating}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, fontFamily: MONO, color: '#001018', background: 'var(--cyan)', border: 'none', borderRadius: 6, padding: '6px 11px', cursor: validating ? 'default' : 'pointer', opacity: validating ? 0.6 : 1 }}>
                {validating ? <Loader2 size={12} className="spin" /> : <GitBranch size={12} />} {validating ? 'Validating…' : 'Validate with Digital Twin'}
              </button>
            </div>
            ...
            {twin && !twin.available && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5, color: 'var(--text-muted)' }}>
                <WifiOff size={13} /> Digital Twin has no baseline for <b style={{ color: 'var(--text)' }}>&nbsp;{ctx.circuitName}</b>&nbsp;or is asleep — local model stands.
              </div>
            )}
            ...
            {twin && twin.available && (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                  ...
                  {twin.approvalRequired && <span style={{ fontSize: 10, fontFamily: MONO, color: 'var(--yellow)' }}>· approval required</span>}
                </div>
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 8 }}>
                  <TwinMetric label="Lap Δ" value={`${(twin.lapDeltaS ?? 0) <= 0 ? '' : '+'}${(twin.lapDeltaS ?? 0).toFixed(3)} s`} color={(twin.lapDeltaS ?? 0) < 0 ? 'var(--green)' : (twin.lapDeltaS ?? 0) > 0 ? 'var(--accent)' : 'var(--text)'} />
                  <TwinMetric label="Stability Δ" value={(twin.stabilityDelta ?? 0).toFixed(3)} color={(twin.stabilityDelta ?? 0) >= 0 ? 'var(--green)' : 'var(--accent)'} />
                  <TwinMetric label="Rear temp Δ" value={`${(twin.rearTempDeltaC ?? 0).toFixed(2)} °C`} color="var(--text)" />
                </div>
                ...
                {twin.limitations && twin.limitations.length > 0 && (
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 6 }}>
                    {twin.limitations.map(l => <div key={l}>· {l}</div>)}
                  </div>
                )}
              </div>
            )}
          ...
          <div style={{ fontSize: 10.5, color: 'var(--text-muted)', lineHeight: 1.5 }}>
            Instant local model — same levers, same outcome — for exploration; the Digital Twin button runs the real simulation lab for a verifiable second opinion. A scenario here is a hypothesis; validate it on track before committing to the bike (Setup Lab).
          </div>
        </div>
      </div>
    </div>
