...
const LEVERS: LeverDef[] = [
  ...
  { key: 'trackTempDelta', label: 'Track temp', min: -10, max: 10, step: 1, unit: '─š\u00b0C', fmt: v => (v > 0 ? `+${v}` : `${v}`) },
            ...
            {twin && twin.available && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 8 }}>
                  <TwinMetric label="Lap →" value={`${(twin.lapDeltaS ?? 0) <= 0 ? '' : '+'}${(twin.lapDeltaS ?? 0).toFixed(3)} s`} color={(twin.lapDeltaS ?? 0) < 0 ? 'var(--green)' : (twin.lapDeltaS ?? 0) > 0 ? 'var(--accent)' : 'var(--text)'} />
                  <TwinMetric label="Stability →" value={(twin.stabilityDelta ?? 0).toFixed(3)} color={(twin.stabilityDelta ?? 0) >= 0 ? 'var(--green)' : 'var(--accent)'} />
                  <TwinMetric label="Rtemp →–" value={`${(twin.rearTempDeltaC ?? 0).toFixed(2)} ─š\u00b0C`} color="var(--text)" />
                </div>
...
export default ScenarioSandboxPage;
