    ...
    <div className="page">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 14 }}>
        <div className="card" style={{ padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}><Trophy size={14} style={{ color: 'var(--cyan)' }} /><span style={hdr}>Standings · last 5</span></div>
          {c.standings.map(s => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginTop: 14 }}>
        <div className="card" style={{ padding: 16, borderLeft: `3px solid ${engineColor(c.engine.status)}` }}>
          ...
          <div style={{ fontSize: 9, fontFamily: MONO, color: engineColor(c.engine.status), textTransform: 'uppercase' }}>{c.engine.status} · {c.engine.avgMileageKm} km avg</div>
      ...
      <div style={{ fontSize: 9, fontFamily: MONO, color: 'var(--text-muted)', marginTop: 14, fontStyle: 'italic' }}>
        Representative season model · 37 pts/weekend (25 GP + 12 Sprint win). Not a live results feed.
      </div>
    </div>
  );
}
