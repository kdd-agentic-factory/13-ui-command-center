...
const RIVAL_PROFILES: RivalProfile[] = [
  ...
  },
  {
    ...
    trend: 'improving',
    threat: 'Self',
    strategy: 'Protect rear L1–L5, attack P2 only if gap <0.6s.',
    self: true,
  },
  {
    grid: 'P4',
...
const DATA_SOURCES: DataSource[] = [
  ...
  { source: 'Setup sheets', status: 'FP1–Q2 versions loaded', quality: '94%' },
];
  ...
  return (
    <svg width="100%" height={H + 4} viewBox={`0 0 ${W} ${H + 4}`} preserveAspectRatio="xMidYMid meet" style={{ display: 'block' }}>
      ...
      {PACE_EVOLUTION.map((item, index) => {
        ...
        return (
          <g key={item.session}>
            <circle cx={cx} cy={cy} r={isBest ? 5 : 3.5} fill={isBest ? 'var(--green)' : '#3B82F6'} stroke="rgba(255,255,255,0.18)" strokeWidth="2" />
            <text x={cx} y={H - 4} textAnchor="middle" fill={isBest ? 'var(--green)' : '#535A6E'} fontSize="8" fontFamily="JetBrains Mono,monospace" fontWeight={isBest ? 800 : 500}>{item.session}</text>
          </g>
        );
      })}
    </svg>
  ...
  return (
    <div className="page">
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6 }}>
              ...
              <strong style={{ color: 'var(--text)' }}>Strategic recommendation:</strong> start Front Medium / Rear Soft. Protect rear L1–L5. Attack P2 only if gap &lt;0.6s before San Donato.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="card-body">
          <div className="grid-4" style={{ marginBottom: 16 }}>
            <StatTile label="Priority 1" value="Protect rear L1–L5" color="var(--green)" />
