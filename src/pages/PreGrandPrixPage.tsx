...
const CIRCUIT_LIBRARY: CircuitLibraryItem[] = [
  ...
  { name: 'MontmelÃƒÂ³', id: 'montmelo', loaded: false },
  { name: 'PortimÃƒÂ£o', id: 'portimao', loaded: false },
  { name: 'Misano', id: 'misano', loaded: false },
  { name: 'MotorLand AragÃƒÂ³n', id: 'aragon', loaded: false },
...
const WEEKEND_SCHEDULE: ScheduleSession[] = [
  { session: 'FP1', day: 'Fri', time: '09:00', status: SESSION_STATUS.COMPLETED, result: 'P3 Ã‚Â· 1:44.102', dataQuality: '96%', note: 'Setup baseline created' },
  { session: 'FP2', day: 'Fri', time: '14:00', status: SESSION_STATUS.COMPLETED, result: 'P5 Ã‚Â· 1:43.847', dataQuality: '94%', note: 'Rear tyre thermal load detected' },
  { session: 'FP3', day: 'Sat', time: '09:30', status: SESSION_STATUS.COMPLETED, result: 'P4 Ã‚Â· 1:43.612', dataQuality: '97%', note: 'Race pace simulation updated' },
  { session: 'Q1', day: 'Sat', time: '14:30', status: SESSION_STATUS.NOT_REQUIRED, result: 'Q2 direct', dataQuality: 'Ã¢â‚¬â€', note: 'No additional run required' },
  { session: 'Q2', day: 'Sat', time: '15:00', status: SESSION_STATUS.COMPLETED, result: 'P3 Ã‚Â· 1:43.201', dataQuality: '98%', note: 'Grid row 1' },
  { session: 'Race', day: 'Sun', time: '14:00', status: SESSION_STATUS.PENDING, result: 'Pending', dataQuality: 'ready', note: `Grid P3 Ã‚Â· planned race distance ${MUGELLO_CIRCUIT.raceLaps} laps` },
];

const PACE_EVOLUTION: PaceEvolution[] = [
  { session: 'FP1', lap: '1:44.102', setup: 'baseline setup', tyres: 'Medium/Soft', trackTemp: '34Ã‚Â°C', gain: 'REF' },
  { session: 'FP2', lap: '1:43.847', setup: 'rear compression -1', tyres: 'Medium/Soft', trackTemp: '39Ã‚Â°C', gain: '-0.255s' },
  { session: 'FP3', lap: '1:43.612', setup: 'engine brake EB4', tyres: 'Medium/Soft', trackTemp: '31Ã‚Â°C', gain: '-0.490s' },
  { session: 'Q2', lap: '1:43.201', setup: 'qualifying map', tyres: 'Soft/Soft', trackTemp: '42Ã‚Â°C', gain: '-0.901s' },
];
...
const RIVAL_PROFILES: RivalProfile[] = [
  {
    ...
    pace: '1:43.04',
    tyre: 'M/S',
    strength: 'Sector 3 Ã‚Â· Bucine exit',
  ...
  },
  {
    ...
    pace: '1:43.18',
    tyre: 'M/M',
    strength: 'Sector 1 Ã‚Â· San Donato braking',
  ...
  },
  {
    ...
    pace: '1:43.41',
    tyre: 'M/S',
    strength: 'Sector 2 Ã‚Â· Arrabbiata flow',
    weakness: 'Bucine exit traction',
    trend: 'improving',
    threat: 'Self',
    strategy: 'Protect rear L1Ã¢â‚¬â€œL5, attack P2 only if gap <0.6s.',
    self: true,
  },
  {
    grid: 'P4',
    ...
    trend: 'aggressive',
    threat: 'Medium-high L1Ã¢â‚¬â€œL8',
...
const DATA_SOURCES: DataSource[] = [
  ...
  { source: 'Elevation model', status: 'Procedural gradient brief Ã‚Â· no real mesh asset loaded', quality: 'procedural' },
  { source: 'Historical telemetry', status: 'FP1 Ã‚Â· FP2 Ã‚Â· FP3 Ã‚Â· Q2 loaded', quality: '96%' },
  ...
  { source: 'Setup sheets', status: 'FP1Ã¢â‚¬â€œQ2 versions loaded', quality: '94%' },
];
    ...
    <div className="page">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="page-subtitle">Circuit loading Ã‚Â· weekend planning Ã‚Â· race simulation Ã‚Â· setup baseline Ã‚Â· rival intelligence</p>
        </div>
      ...
      <div className="card mb-4" style={{
        ...
        <div className="card-header">
          <span className="card-title">GP {MUGELLO_CIRCUIT.shortName} Ã‚Â· {MUGELLO_CIRCUIT.country} Ã‚Â· {MUGELLO_CIRCUIT.season} Season</span>
          <span className="badge badge-yellow">Race not started Ã‚Â· preparation phase</span>
        </div>
        <div className="card-body">
          <div className="grid-4" style={{ marginBottom: 18 }}>
            <StatTile label="Status" value="PRE-RACE" color="var(--green)" />
            <StatTile label="Circuit" value={`${MUGELLO_CIRCUIT.lengthKm} km Ã‚Â· ${MUGELLO_CIRCUIT.turns} turns`} color="var(--blue)" />
            <StatTile label="Race distance" value={`${MUGELLO_CIRCUIT.raceLaps} laps`} color="var(--text)" />
            <StatTile label="Digital Twin" value="Ready Ã‚Â· 4 scenarios" color="var(--yellow)" />
          </div>
          <PreparationProgress />
        </div>
      </div>
      ...
      <div className="grid-2 mb-4">
          ...
          <div className="card-body">
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
              {CIRCUIT_LIBRARY.map(circuit => {
                ...
                return (
                  <button key={circuit.id} type="button" disabled={!circuit.loaded} onClick={() => setSelectedCircuit(circuit.name)} title={circuit.loaded ? 'Loaded for the active session circuit' : 'Not loaded Ã¢â‚¬â€ selecting would mismatch the active session circuit'} style={{ border: `1px solid ${active ? 'var(--blue)' : !circuit.loaded ? 'rgba(245,158,11,0.28)' : 'var(--border)'}`, background: active ? 'rgba(59,130,246,0.12)' : 'rgba(255,255,255,0.03)', color: active ? 'var(--blue)' : !circuit.loaded ? 'var(--yellow)' : 'var(--text-muted)', borderRadius: 999, padding: '6px 10px', fontSize: 10, fontFamily: 'JetBrains Mono,monospace', fontWeight: active ? 800 : 500, cursor: circuit.loaded ? 'pointer' : 'not-allowed', opacity: circuit.loaded ? 1 : 0.72 }}>
                    {circuit.name}{!circuit.loaded ? ' Ã‚Â· not loaded' : ''}
                  </button>
                );
              })}
            </div>
        ...
        <div className="card">
          <div className="card-header">
            <span className="card-title flex items-center gap-2"><Map size={14} />Circuit Intelligence Ã‚Â· {getSessionContext().circuitName}</span>
            <span className="badge badge-blue">MotoGP layout</span>
          </div>
          <div className="card-body">
            <div className="grid-4" style={{ marginBottom: 16 }}>
              <StatTile label="Length" value={`${MUGELLO_CIRCUIT.lengthKm} km`} />
              <StatTile label="Turns" value={`${MUGELLO_CIRCUIT.turns} Ã‚Â· ${MUGELLO_CIRCUIT.leftTurns}L / ${MUGELLO_CIRCUIT.rightTurns}R`} />
            ...
            <div className="grid-2">
              <div>
                ...
                {[
                  'T1 San Donato Ã‚Â· heavy braking after main straight',
                  'T3 Poggio Secco Ã‚Â· high point / crest',
                  'T8/T9 Arrabbiata Ã‚Â· high-speed lean load',
                  'T12 Correntaio Ã‚Â· braking stability',
                  'T15 Bucine Ã‚Â· exit onto main straight',
                ].map(zone => <div key={zone} style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 7 }}>Ã¢â‚¬Â¢ {zone}</div>)}
              </div>
              <div>
                ...
                ].map(driver => <div key={driver} style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 7 }}>Ã¢â‚¬Â¢ {driver}</div>)}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid-2 mb-4">
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            {[
              'Main straight crest Ã¢â€ â€™ San Donato braking',
              'Casanova/Savelli downhill transition',
              'Arrabbiata 1/2 high-speed load',
              'Bucine exit onto main straight',
            ].map(item => <div key={item} style={{ fontSize: 12, color: 'var(--text-muted)', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>Ã¢â‚¬Â¢ {item}</div>)}
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <div style={{ marginTop: 12 }}>
              {PACE_EVOLUTION.map(item => (
                <div key={item.session} style={{ display: 'grid', gridTemplateColumns: '42px 80px 1fr 88px', gap: 10, alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{item.setup} Ã‚Â· {item.tyres} Ã‚Â· track {item.trackTemp}</span>
            ...
            <div style={{ marginTop: 12, fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.55 }}>
              <strong style={{ color: 'var(--green)' }}>Main gain:</strong> Sector 2 Ã‚Â· -0.420s. <strong style={{ color: 'var(--yellow)' }}>Remaining weakness:</strong> Sector 3 Ã‚Â· Bucine exit.
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div className="grid-3" style={{ marginBottom: 14 }}>
              {[
                { label: 'Race start', value: '14:00 Ã‚Â· 28Ã‚Â°C Ã‚Â· rain 5%', icon: <CloudSun size={18} style={{ color: 'var(--yellow)' }} /> },
                { label: 'Mid race', value: '15:00 Ã‚Â· 28Ã‚Â°C Ã‚Â· rain 10%', icon: <Wind size={18} style={{ color: 'var(--text-muted)' }} /> },
                { label: 'Late race', value: '16:00 Ã‚Â· 26Ã‚Â°C Ã‚Â· rain 35%', icon: <CloudRain size={18} style={{ color: 'var(--blue)' }} /> },
              ].map(item => (
          ...
          <div className="card-body">
            <div className="grid-2" style={{ marginBottom: 14 }}>
              <div>
                ...
                {['Soft: 2 new / 1 used', 'Medium: 3 new / 1 used', 'Hard: 2 new / 1 used', 'Wet: approved'].map(row => <div key={row} style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 6 }}>Ã¢â‚¬Â¢ {row}</div>)}
              </div>
              <div>
                <div className="card-label" style={{ marginBottom: 8 }}>Rear</div>
                {['Soft: 3 new / 0 used', 'Medium: 2 new / 2 used', 'Hard: 2 new / 1 used', 'Wet: approved'].map(row => <div key={row} style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 6 }}>Ã¢â‚¬Â¢ {row}</div>)}
              </div>
            </div>
            <div className="grid-3">
              <StatTile label="Race plan" value="F Medium Ã‚Â· R Soft" color="var(--green)" />
              <StatTile label="Backup dry" value="F Medium Ã‚Â· R Hard" color="var(--yellow)" />
            ...
            <div style={{ marginTop: 14, padding: '10px 12px', border: '1px solid rgba(224,55,55,0.18)', background: 'rgba(224,55,55,0.07)', borderRadius: 8, fontSize: 12, color: 'var(--text-muted)' }}>
              <strong style={{ color: 'var(--accent)' }}>Tyre risk:</strong> rear soft thermal cliff from L13 if track temperature exceeds 46Ã‚Â°C.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid-2 mb-4">
        <div className="card">
          ...
          <div className="card-body">
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6 }}>
              <strong style={{ color: 'var(--text)' }}>Electronics:</strong> Launch MAP-4 Ã‚Â· Race MAP-6 Ã‚Â· TC race TC4 Ã‚Â· Start TC6.<br />
        ...
        <div className="card">
          <div className="card-header">
            ...
            <span className="badge badge-green">Completed Ã‚Â· 87% confidence</span>
          </div>
          <div className="card-body">
            <div className="grid-3" style={{ marginBottom: 16 }}>
              <StatTile label="Qualifying" value="P3 Ã‚Â· front row" color="var(--yellow)" />
              <StatTile label="Race projection" value="P2Ã¢â‚¬â€œP3" color="var(--green)" />
              <StatTile label="Simulation" value={`${MUGELLO_CIRCUIT.raceLaps} laps Ã‚Â· 5k MC`} color="var(--blue)" />
            </div>
            <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6 }}>
              ...
              <strong style={{ color: 'var(--text)' }}>Strategic recommendation:</strong> start Front Medium / Rear Soft. Protect rear L1Ã¢â‚¬â€œL5. Attack P2 only if gap &lt;0.6s before San Donato.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card mb-4">
        <div className="card-header">
          <span className="card-title flex items-center gap-2"><Target size={14} />Rival Intelligence Ã‚Â· KDD Scout Agent</span>
      ...
      <div className="grid-3 mb-4">
        <div className="insight-panel" style={{ ['--dot-color' as string]: 'var(--yellow)' }}>
          <div className="insight-panel__title" style={{ color: 'var(--yellow)' }}>Tyre Strategy Ã‚Â· Dry Race Mode</div>
          <p className="insight-panel__body">Recommended Front Medium / Rear Soft. Objective: manage rear soft to race distance. Risk begins around L13. Backup: Front Medium / Rear Hard if track temp exceeds 48Ã‚Â°C.</p>
        </div>
        <div className="insight-panel" style={{ ['--dot-color' as string]: 'var(--blue)' }}>
          <div className="insight-panel__title" style={{ color: 'var(--blue)' }}>Sector Opportunity Ã‚Â· S2</div>
          <p className="insight-panel__body">Target Casanova Ã¢â€ â€™ Savelli Ã¢â€ â€™ Arrabbiata 1/2. Gap to P2: +0.08s. Improve throttle pickup out of Savelli and reduce steering correction through Arrabbiata 2.</p>
        </div>
        <div className="insight-panel" style={{ ['--dot-color' as string]: 'var(--green)' }}>
          <div className="insight-panel__title" style={{ color: 'var(--green)' }}>Grid Start Strategy</div>
          <p className="insight-panel__body">P3 Row 1 Ã‚Â· Launch MAP-4 Ã‚Â· Start TC6 Ã‚Â· clutch slip 8%. Plan A: hold inside into San Donato. Plan B: outside cutback if boxed in. Do not overheat rear with launch spin.</p>
        </div>
      </div>
      ...
      <div className="grid-2 mb-4">
        <div className="card">
          ...
          <div className="card-body" style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.7 }}>
            ...
            <strong style={{ color: 'var(--text)' }}>Tyres:</strong> wet approved Ã‚Â· intermediate standby.<br />
            <strong style={{ color: 'var(--text)' }}>Electronics:</strong> Rain MAP-2 Ã‚Â· TC rain TC8 Ã‚Â· EB5.<br />
      ...
      <div className="card">
        ...
        <div className="card-body">
          <div className="grid-4" style={{ marginBottom: 16 }}>
            <StatTile label="Priority 1" value="Protect rear L1Ã¢â‚¬â€œL5" color="var(--green)" />
