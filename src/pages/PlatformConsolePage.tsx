 ...
 * experiments and MCP tools — each via the existing live-with-fallback feed
...
export function PlatformConsolePage() {
  ...
  return (
    <div className="page">
      ...
      <div className="card mb-4" style={{ padding: 14, borderLeft: '3px solid var(--violet)' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input value={intent} onChange={e => setIntent(e.target.value)} onKeyDown={e => e.key === 'Enter' && runIntent()}
            ...
            style={{ flex: 1, fontSize: 12, background: 'var(--bg-surface)', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px' }} />
          <button onClick={runIntent} disabled={routing}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, fontFamily: MONO, color: '#001018', background: 'var(--violet)', border: 'none', borderRadius: 6, padding: '0 12px', cursor: routing ? 'default' : 'pointer', opacity: routing ? 0.6 : 1 }}>
        ...
        {routed && routed !== 'fail' && (
          <div style={{ marginTop: 8, fontSize: 12, color: 'var(--text)' }}>
            ...
            {routed.agent_used && <> · <span style={{ color: 'var(--text-muted)' }}>routed to </span><b style={{ color: 'var(--violet)' }}>{routed.agent_used}</b></>}
          </div>
        )}
