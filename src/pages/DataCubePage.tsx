      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 }}>
        ...
        {cube.lenses.map(l => (
          <button key={l.id} onClick={() => setLens(l.id)} title={l.desc}
            style={{ fontSize: 10.5, fontFamily: MONO, padding: '4px 9px', borderRadius: 6, cursor: 'pointer',
      ...
      {CUBE_CORNERS.map(c => {
        ...
        return (
          <button key={c} onClick={() => onPick(c)}
            style={{ height: 38, borderRadius: 6, cursor: 'pointer', border: '1px solid var(--border)',
