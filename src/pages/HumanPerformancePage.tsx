/**
 * HumanPerformancePage â€” Human Performance Intelligence.
 *
 ...
 * estimated â€” the value is the correlation to the telemetry model.
 */
    ...
    <div className="page">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="page-subtitle">Rider state Â· fatigue Â· focus Â· confidence Â· cognitive load Â· human-aware risk â€” {h.combo}</p>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 9, fontFamily: MONO, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Rider state Â· conf {h.confidencePct}%</div>
      ...
      <div className="card mb-4" style={{ padding: 14 }}>
        <div style={{ fontSize: 12.5, color: 'var(--text)' }}><b>Status:</b> {h.status} Â· <span style={{ color: 'var(--accent)' }}>{h.mainLimiter}</span></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'start' }}>
        ...
        <div className="card" style={{ padding: 16 }}>
          ...
          {h.evidence.map(e => <div key={e} style={{ fontSize: 10.5, color: 'var(--text-muted)' }}>Â· {e}</div>)}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginTop: 14 }}>
        <div className="card" style={{ padding: 16 }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 6 }}>{h.confidence.evidence.join(' Â· ')}</div>
        </div>
        <div className="card" style={{ padding: 16 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text)', marginTop: 6 }}>Recommended: <b style={{ color: 'var(--green)' }}>{h.readiness.recommendedStint}</b> Â· push: {h.readiness.pushLaps}</div>
        ...
        <div className="card" style={{ padding: 16 }}>
          ...
          {h.events.map(e => (
            <div key={e.type} style={{ marginBottom: 9, paddingBottom: 9, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <div style={{ fontSize: 10, color: 'var(--text-muted)', margin: '2px 0' }}>{e.evidence.join(' Â· ')}</div>
              <div style={{ fontSize: 11 }}><span style={{ color: 'var(--accent)' }}>{e.impact}</span> Â· <span style={{ color: 'var(--cyan)' }}>{e.recommendation}</span></div>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="card" style={{ padding: 16 }}>
            <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: '0.1em', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 8 }}>Before / after Â· human</div>
            {h.beforeAfter.map(r => (
          ...
          <div className="card" style={{ padding: 16,
            ...
            <div style={{ fontSize: 11, fontFamily: MONO, color: 'var(--text-muted)', marginBottom: 6 }}>air {h.wellness.airTempC}Â°C Â· track {h.wellness.trackTempC}Â°C Â· load {h.wellness.riderLoad}</div>
            {h.wellness.warnings.map(w => <div key={w} style={{ fontSize: 11, color: 'var(--text)' }}>Â· {w}</div>)}
          </div>
        </div>
      </div>
